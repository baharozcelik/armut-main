...
class APIManager {
    ...
    static func requestData(url:String,method:HTTPMethod,parameters:parameters?,completion: @escaping (ApiResult)->Void) {
        ...
        var urlRequest = URLRequest(url: URL(string: Constants.baseUrl+url)!, cachePolicy: .useProtocolCachePolicy, timeoutInterval: 10)
        ...
        URLSession.shared.dataTask(with: urlRequest) { (data, response, error) in
            if let error = error {
                ...
                do {
                    ...
                    completion(ApiResult.failure(.unknownError))
                    print("Error when parsing to JSON : \(parseJSONError)")
                }
            }
        }.resume()
    }
